...
\title[Semester project]{Verification of Formal Languages} % The short title appears at the bottom of every slide, the full title is only on the title page
...
\begin{frame}
	...
	\only<3> {
		\begin{definition}[Power of languages]
			let $\languagepow{0} = \{ \emptyword \}$ and $\languagepow{i+1} = \languagesymbol \concatop \languagepow{i} $
		\end{definition}
	}
	\only<4> {
...
\begin{center}
	\begin{table}
		...
		\begin{tabular}{| p{2cm} | p{2cm} | p{2cm}| p{2cm} | p{2cm} |}
			...
			Equality & Trivial & Content = & Content = &\highlightbad{ $\forall$ expr} \\ \hline
...
\begin{frame}{Sounds good, doesn't work?}
	Every theorem about words and languages has been verified by Stainless.

	\onslide<2->{About regular expressions, only the exponent positiveness lemma has been verified, the other has been proven for the constant and union cases.}	

	\onslide<3->{Every verifying theorem and lemma was cleared to be terminating. }
	
	\onslide<4->{Metrics (excluding lemmas about regular expressions):
		\begin{itemize}
			\item 95 functions (methods and lemmas)
			\item 651 verified conditions
			\item 5127.148s of total time for verification (longest: 318.947s)
			\item 1936.337s of total time for checking termination 
		\end{itemize}
		
		CPU: Dual-Core Intel® Core™ i5-4210U CPU @ 1.70GHz processor RAM: 8GB}
\end{frame}

\begin{frame}{Conclusion and Future Work}
	Summary:
	\begin{itemize}
		\item analyzed different language representations
		\item presented a formal language implementation using lists
		\item based on that implementations proved properties of languages
		\item applied implemented abstractions to represent regular expressions and prove theorems about them
	\end{itemize}
	...
	Possible future work:
		\begin{itemize}
			\item Experiment with other language representation
			\item Finish incomplete lemmas about regular expressions
			\item Try to prove further statements about regular expressions
			\item Examine other abstractions using formal languages
		\end{itemize}}
\end{frame}
